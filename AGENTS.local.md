...
- **E2E 자동화 (T-98)**: `./scripts/e2e/run-e2e.sh` — 로컬 서버 격리 + 실제 Whale + 실제 확장으로 추출→저장→표시 전체 파이프라인 검증 (연속 실행 가능, 자동 정리)
- **몰 주의**: 네이버 스마트스토어는 자동화 접속 시 HTTP 429 봇 차단 가능 (E2E 기본 상품은 올리브영)
