...
> 목적: 웨일 스토어 리스팅에 올릴 스크린샷 5장을 준비한다.
> 대상 파일: `docs/screenshots/store/shop-wisebar-01.png` (상품 페이지 + 플로팅) · `-02.png` (팝업 현재 상품 탭) · `-03.png` (팝업 핫딜 30일 탭) · `-04.png` (플로팅 메뉴 펼침) · `-05.png` (가격 추이 패널)
...
2. 현재 상품 URL이 서버에 있으면 **임시 과거 가격 포인트(2일 이상)만 추가**해 가격 추이를 만듦
   (v0.12.3 — 캡처 후 추가분만 삭제해 실데이터 보존), 없으면 가격 이력 3개 주입
...
4. 스크린샷 5장 캡처:
   ...
   - ④ 상품 페이지 1280×800 — **플로팅 메뉴 펼침** (v0.12.3)
   - ⑤ 상품 페이지 1280×800 — **가격 추이 패널** (v0.12.3)
5. **데모 데이터 자동 삭제** (추가한 상품 전부 DELETE) + 임시 가격 포인트만 삭제 (실데이터 보존)
...
- `shop-wisebar-04.png` — 1280×800 (플로팅 메뉴 펼침 — 메뉴 5개 아이콘)
- `shop-wisebar-05.png` — 1280×800 (가격 추이 패널 — 2일 이상 이력 그래프)
- 팝업 화면 상태(상품명·가격·핫딜 목록)는 실행 로그의 `▸ 팝업 상태:` JSON에서 확인 가능
- 플로팅 버튼/메뉴는 실행 로그의 `✓ 플로팅 버튼 표시 확인` / `메뉴 아이콘이 화면 내부` 메시지로 확인 가능
...
- [ ] `shop-wisebar-04.png` — 1280×800, 플로팅 메뉴(핫딜/알림/추이/찜/설정) 5개가 펼쳐 보이는지 (v0.12.3)
- [ ] `shop-wisebar-05.png` — 1280×800, 가격 추이 그래프(2일 이상 이력)가 보이는지 (v0.12.3)
