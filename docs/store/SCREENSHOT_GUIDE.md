...
- 기본 상품 URL: 네이버 스마트스토어 상품 예시
- 다른 상품으로 캡처하려면 URL을 인자로 전달:

```bash
node capture.js "https://smartstore.naver.com/{스토어}/products/{상품번호}"
node capture.js "https://www.coupang.com/vp/products/{상품번호}"
```

> ⚠ 쿠팡 상품 URL은 봇 차단(403)으로 열리지 않을 수 있다. 네이버 스마트스토어 권장.
...
4. 팝업을 열어 ① 현재 상품 탭 ② 핫딜 "7일" 탭 순으로 캡처
...
- `shop-wisebar-01.png` — 320×600 (현재 상품 탭)
- `shop-wisebar-02.png` — 320×600 (핫딜 탭)
...
1. 네이버 스마트스토어/쿠팡 상품 페이지를 연다 (가격 이력이 쌓인 상품 권장)
...
- [ ] `shop-wisebar-01.png` — 상품명·가격·최저가 요약이 보이는지
- [ ] `shop-wisebar-02.png` — 핫딜 상품 3개 이상 + 하락% 배지가 보이는지
