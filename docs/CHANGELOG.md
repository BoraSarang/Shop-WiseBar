...
## v0.16.20 (2026-08-15) — [all] 프로젝트 종료(아카이브)
> **사용자 결정**: 프로젝트 접기. 타사 스크래핑(쿠팡 Akamai/네이버 캡차/올리브영 WAF)과 가격 오탐의
> 구조적 한계로 유지보수 비용이 지속 가치를 넘어선다고 판단. 저장소는 읽기 전용으로 보존.
- [docs] **종료 아카이브** — `docs/ARCHIVE.md` 작성 (종료 사유, 시스템 구성, 기술 스택, 실행 방법, 운영 인프라, 실측 교훈, 재사용 체크리스트)
- [docs] README.md 상단에 아카이브 상태 배너 추가
- [docs] `docs/ARCHIVE.md` 8장에 가격 오탐 근본 원인(검증 계층 부재)과 방어 설계(하한/상한/급변/소스 신뢰도) 기록
- [meta] 운영 인프라(Render/Neon/스토어) 정리 시 `docs/ARCHIVE.md` 7장 참고
