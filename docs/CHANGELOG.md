...
## [0.2.4] — 2026-08-03 [macos]+[server] — T-58 추천 리스트

### 추가
- 서버: GET /api/v1/recommendations — 최근 7일 이내 가격 하락 상품 (직전 포인트 대비), 하락폭 큰 순. 응답에 drop_amount/previous_price 포함 (RecommendationOut)
- 클라이언트: 홈 모드 "추천 상품 — 최근 가격 하락" 섹션 (최대 3개) — 이미지/이름/현재가/▼하락 배지, 탭 시 브라우저 열기. 팝오버 열림마다 재조회

### 검증 (실기기)
- 서버 스모크: teststore 999999 (▼10,000) / 123456 (▼5,000) 하락폭순 반환 — PASS
- 홈 모드 추천 섹션 표시 + 클릭 열기 — PASS (스크린샷 v0_3_t58_recommend.png)
