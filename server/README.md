...
.venv/bin/python -m crawlers.worker      # 올리브영·네이버 크롤러 워커 (별도 프로세스)
```
...
## 수집 우선순위 (v0.16.0 — 2026-08-10 2차 검증 반영)

1. 서버 크롤러 — 올리브영 + **네이버 브랜드스토어** (Playwright headless, 실측 성공)
2. 브라우저 익스텐션 — 네이버/쿠팡/올리브영 전부 (상품 페이지 방문 시 DOM 추출 → 업로드, source=extension)
3. (폐기) 맥 메뉴바 osascript — 2026-08-03 제거

- 쿠팡은 서버 자동 수집 불가 (Akamai) → 익스텐션 의존 유지. 검증 기록: `docs/tests/v0.3_crawler_poc.md` + `ShopWiseBar-Verify/results/verify-report.md`
